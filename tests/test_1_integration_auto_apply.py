...
def test_immediate_execution(mocker, config_file):
    """
    測試立即執行模式。
    使用 Mock Logger 攔截訊息。
    """
    ...
    runner = CliRunner()
    result = runner.invoke(app, ["--config", config_file])
...
def test_scheduled_execution(mocker, config_file):
    """
    測試排程執行模式。
    使用 Mock Logger 攔截訊息。
    """
    ...
    runner = CliRunner()
    result = runner.invoke(app, ["-c", config_file, "-d", execute_time_str])
    assert result.exit_code == 0
